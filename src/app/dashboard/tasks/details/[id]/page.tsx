...
// ─── Database fetchers ────────────────────────────────────────────────────────
type TSelection = {
  variabel_penilaian_id: any;
  opsi_nilai_bobot_id: any;
  variabel_penilaian?: any;
  opsi_nilai_bobot_variabel?: any;
};
...
async function fetchTaskSelections(taskId: string): Promise<TSelection[]> {
  const { data, error } = await supabase
  ...
  return (data ?? []).map((item) => ({
    variabel_penilaian_id: item.variabel_penilaian_id,
    opsi_nilai_bobot_id: item.opsi_nilai_bobot_id,
    variabel_penilaian: item.variabel_penilaian ?? null,
    opsi_nilai_bobot_variabel: item.opsi_nilai_bobot_variabel ?? null,
  }));
}

async function markTaskSelesai(taskId: string | null) {
...
export default function DetailKegiatan({
  ...
  const getSelection = (variabelId: number) =>
  ...
  return (
    <>
      ...
      <div className="dk-page">
        {/* Back */}
        <button className="dk-back" onClick={() => router.back()}>
        ...
        {task.deskripsi && (
          <div className="dk-section">
            ...
            <div className="dk-note">{task.deskripsi}</div>
          </div>
        )}
        ...
        <div className="dk-actions">
          <button
            ...
            style={
              isSelesai ? { background: "#10b981", cursor: "default" } : {}
            }
          >
            {marking
              ? "Menyimpan..."
...
function StatCard({ label, value, sub, valueStyle = {} }: TStatCard) {
  return (
    <div className="dk-stat-card">
      <span className="dk-stat-label">{label}</span>
      <span className="dk-stat-value" style={valueStyle}>
        {value}
      </span>
  ...
  return (
    <div className="dk-page">
      <div className="dk-error-box">
        ...
        <button className="dk-btn-secondary" onClick={onBack}>
          ← Kembali
        </button>
      </div>
    </div>
