...
function Card({
  ...
  return (
    <div className={`analytics-card glass-card ${className}`}>{children}</div>
  );
}
...
function SummaryCard({
  ...
  return (
    <Card>
      <p style={{ color: "var(--muted)", fontWeight: 800, marginBottom: 10 }}>
        {label}
      </p>
...
export default function DashboardAnalitik() {
  ...
  const barData = useMemo(() => {
    const map: Record<
      number,
      { bulan: string; Selesai: number; Menunggu: number }
    > = {};
    rows.forEach((r) => {
      const d = new Date(r.created_at);
      const key = d.getMonth(); // 0-11 untuk Jan-Des
      if (!map[key]) map[key] = { bulan: BULAN[key], Selesai: 0, Menunggu: 0 };
      r.status === "selesai" ? map[key].Selesai++ : map[key].Menunggu++;
    });
    return Object.values(map).sort(
      (a, b) => BULAN.indexOf(a.bulan) - BULAN.indexOf(b.bulan),
    );
  }, [rows]);
  ...
  const pieData = useMemo(() => {
    ...
    rows.forEach((r) => {
      map[
        getPriorityClass(Number(r.total_priority_score || 0), undefined, true)
      ]++;
    });
    return PIE_CONFIG.map((c) => ({
      name: c.label,
      value: map[c.label],
      color: c.color,
    })).filter((d) => d.value > 0);
